...
load('C:\Umaine Google Sync\Masters Working Folder\1 - OpenFAST\Simulations\5MW_OC4Semi_WSt_WavesWN\Turbulent_13mps_Monhegan_NoWave\5MW_OC4Semi_WSt_WavesWN_FAST_Results.mat');
...
for i = 1:length(t)
    ...
    y = [sim_results.QD_P(i);
         sim_results.QD_TFA1(i)];

    % Update state vector
    xp = RK4(dt, x, @FloatingTrinityRHS, F);
    % xp = SRCUKF(dt, x, @FloatingTrinityRHS, @FloatingTrinityMeasurement,Q,R,y,F);
